...
CRUMINA.Bootstrap = function () {
	 ...
	 * https://v4-alpha.getbootstrap.com/getting-started/introduction/
	 * ---------------------------*/
	var date_select_field = $('input[name="birth-date"]');
...
$('#top-search').selectize({
	...
	options: [
		...
		{image: 'assets/img/avatar45-sm.jpg', name: 'Marie Davidson', message:'4 Friends in Common', icon:'olymp-happy-face-icon'},
	...
	render: {
		option: function(item, escape) {
			return '<div class="inline-items">' +
				...
				'<br>'+(item.message ? '<span class="chat-message-item">' + escape(item.message) + '</span>' : '') +
				'</div>'+
...
$(document).ready(function(){
    ...
    function makeProgress(){
        if(i < 100){
            i = i + 1;
            $(".post-progress-bar").css("width", i + "%").text(i + " %");
        }
...
content: document.querySelector('.drop'),
...
target: document.querySelector('.post__author-name'),
